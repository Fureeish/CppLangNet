...
# `std::forward()` function template

Defined in header `<utility>`.

### Description

This utility function template is used to preserve the correct value category of a parameter that is being passed down to another function
that can be overloaded on the value category said parameter — most frequently in a wrapper-style delegation.
...
- Writing a factory function which will ultimately call some constructor (instead of using it directly without any indirection) 
  that cares whether it was called with an rvalue or an lvalue.
- Writing a **single** constructor which handles **both** lvalue and rvalue arguments and forwards them to member-initialzier list
  in order to efficiently construct the fields of the class (instead of overloading said constructor on value categories of its parameters).
...
More formally, even if the *type* of an argument is an rvalue reference, the *value category* of that reference is an lvalue.

To fix the presented issue, we can use `std::forward()`:
...
#### Implementing a constructor that efficiently works with both lvalues and rvalues
...
This implementation is fine, but `2)` will suffer from performance penalties. 
C-string literals (`"Foo"` and `"Bar"`) will first have to be converted to `std::string` temporaries. 

Temporaries can bind to `const&`, so the code compiles, but it is very far from optimal. 
Temporaries will be used to create copies inside `p2`, despite the fact that they could simply be moved into the object.

To fix this, `person`'s constructor can be overloaded so that it accepts rvalues and moves them into class fields:
...
[Previous example](#wrapping-a-function-with-a-logger) illustrated that in this case, `name`'s and `surname`'s value categories are lvalues (even though they bound to temporaries and their types are rvalue references),
so using them to initialize `name` and `surname` fields will not invoke move-constructors. It is necessary to add `std::move` in both cases.
...
We turned the constructor (not the whole class) into a template, so that it will accept any type (be it `std::string` or a c-string literal).

The tricky part is what to put instead of `/* ??? */` in the initialization of the fields. 
...
Despite that, `name` and `surname` are parameters that have names and thus, since we can name them, their value categories are lvalues.
To correctly forward them to the construction of class' fields, we use `std::forward()`.